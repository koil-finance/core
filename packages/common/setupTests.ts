...
import { NAry } from '@koil-finance/helpers/src/models/types/types';
import { ZERO_ADDRESS } from '@koil-finance/helpers/src/constants';
import { KoilErrors } from '@koil-finance/helpers/src/utils/errors';
import { BigNumberish, bn, fp } from '@koil-finance/helpers/src/numbers';
import { expectEqualWithError, expectLessThanOrEqualWithError } from '@koil-finance/helpers/src/test/relativeError';
    ...
    return async function (this: any) {
      ...
      try {
        ...
        try {
          ...
          // Decode the actual revert reason and look for it in the koil errors list
          const regExp =
          ...
          if (KoilErrors.isErrorCode(actualErrorCode)) {
            ...
            if (actualErrorCode.includes('KOIL#')) {
              ...
              // If it's not a Koil error then rethrow
              throw error;
            }
          }

          let expectedErrorCode: string;
          if (KoilErrors.isKoilError(expectedReason)) {
            expectedErrorCode = KoilErrors.encodeError(expectedReason);
          } else {
            // If there is no koil error matching the expected revert reason re-throw the error
