const koilErrorCodes: Record<string, string> = {
  '000': 'ADD_OVERFLOW',
  '001': 'SUB_OVERFLOW',
  '002': 'SUB_UNDERFLOW',
  '003': 'MUL_OVERFLOW',
  ...
  '206': 'UNINITIALIZED',
  '207': 'KPT_IN_MAX_AMOUNT',
  '208': 'KPT_OUT_MIN_AMOUNT',
  '209': 'EXPIRED_PERMIT',
  '210': 'NOT_TWO_TOKENS',
  '300': 'MIN_AMP',
  '301': 'MAX_AMP',
  ...
  '304': 'MAX_IN_RATIO',
  '305': 'MAX_OUT_RATIO',
  '306': 'MIN_KPT_IN_FOR_TOKEN_OUT',
  '307': 'MAX_OUT_KPT_FOR_TOKEN_IN',
  ...
  '514': 'INVALID_FUSE_INTERNAL_BALANCE',
  '515': 'INVALID_POST_LOAN_BALANCE',
  '516': 'INSUFFICIENT_FUSE',
  '517': 'UNALLOCATED_FUSE',
  '518': 'FUSE_TRANSFER',
  '519': 'CANNOT_USE_FUSE_SENTINEL',
  '520': 'TOKENS_MISMATCH',
...
export class KoilErrors {
  ...
  static tryParseErrorCode = (error: string): string => {
    try {
      return KoilErrors.parseErrorCode(error);
    } catch {
      return error;
    }
  };
  ...
  /**
   * Tests whether a string is a known Koil error message
   * @param error - a string to be checked verified as a Koil error message
   */
  static isKoilError = (error: string): boolean => Object.values(koilErrorCodes).includes(error);
