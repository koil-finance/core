...
export default class Token {
  ...
  static async deployedAt(address: string): Promise<Token> {
    const instance = await deployedAt('standalone-utils/TestToken', address);
    const [name, symbol, decimals] = await Promise.all([instance.name(), instance.symbol(), instance.decimals()]);
    if (symbol === 'WFUSE') {
      return new Token(name, symbol, decimals, await deployedAt('standalone-utils/TestWFUSE', address));
    }
