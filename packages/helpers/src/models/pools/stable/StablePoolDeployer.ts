...
export default {
  ...
  async _deployStandalone(params: StablePoolDeployment, vault: Vault): Promise<Contract> {
    ...
    return params.meta
      ? deploy('pool-stable/MockMetaStablePool', {
          args: [
            {
              vault: vault.address,
              name: NAME,
  ...
  async _deployFromFactory(params: StablePoolDeployment, vault: Vault): Promise<Contract> {
    ...
    const factory = await deploy('pool-stable/StablePoolFactory', { args: [vault.address], from });
    ...
    return deployedAt('pool-stable/StablePool', event.args.pool);
  },
};
