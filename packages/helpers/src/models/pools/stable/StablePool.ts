...
import { StablePoolEncoder } from '../../../utils/pool-stable';
import {
  Sample,
  MiscData,
  JoinExitStablePool,
...
export default class StablePool {
  ...
  async estimateBptPrice(currentBalances?: BigNumberish[], currentSupply?: BigNumberish): Promise<BigNumber> {
    if (!this.meta) throw Error('KPT price estimation is only available for meta stable pools');
    if (!currentBalances) currentBalances = await this.getBalances();
  ...
  private _buildJoinGivenInParams(params: JoinGivenInStablePool): JoinExitStablePool {
    ...
    return {
      ...
      protocolFeePercentage: params.protocolFeePercentage,
      data: StablePoolEncoder.joinExactTokensInForKPTOut(amountsIn, params.minimumBptOut ?? 0),
    };
  }
  ...
  private _buildJoinGivenOutParams(params: JoinGivenOutStablePool): JoinExitStablePool {
    return {
      ...
      currentBalances: params.currentBalances,
      protocolFeePercentage: params.protocolFeePercentage,
      data: StablePoolEncoder.joinTokenInForExactKPTOut(params.bptOut, this.tokens.indexOf(params.token)),
    };
  }
  ...
  private _buildExitGivenOutParams(params: ExitGivenOutStablePool): JoinExitStablePool {
    ...
    return {
      ...
      currentBalances: params.currentBalances,
      protocolFeePercentage: params.protocolFeePercentage,
      data: StablePoolEncoder.exitKPTInForExactTokensOut(amountsOut, params.maximumBptIn ?? MAX_UINT256),
    };
  }
  ...
  private _buildSingleExitGivenInParams(params: SingleExitGivenInStablePool): JoinExitStablePool {
    return {
      ...
      currentBalances: params.currentBalances,
      protocolFeePercentage: params.protocolFeePercentage,
      data: StablePoolEncoder.exitExactKPTInForOneTokenOut(params.bptIn, this.tokens.indexOf(params.token)),
    };
  }
  ...
  private _buildMultiExitGivenInParams(params: MultiExitGivenInStablePool): JoinExitStablePool {
    return {
      ...
      currentBalances: params.currentBalances,
      protocolFeePercentage: params.protocolFeePercentage,
      data: StablePoolEncoder.exitExactKPTInForTokensOut(params.bptIn),
    };
  }
