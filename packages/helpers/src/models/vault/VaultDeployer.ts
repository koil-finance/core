...
export default {
  ...
  async _deployReal(deployment: VaultDeployment, authorizer: Contract): Promise<Contract> {
    ...
    const args = [authorizer.address, wfuse.address, pauseWindowDuration, bufferPeriodDuration];
    return deploy('vault/Vault', { args, from });
  },

  async _deployMocked({ from }: VaultDeployment, authorizer: Contract): Promise<Contract> {
    return deploy('pool-utils/MockVault', { from, args: [authorizer.address] });
  },

  async _deployAuthorizer(admin: SignerWithAddress, from?: SignerWithAddress): Promise<Contract> {
    return deploy('vault/Authorizer', { args: [admin.address], from });
  },
};
