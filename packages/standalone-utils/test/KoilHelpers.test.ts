...
import { MAX_UINT112, ZERO_ADDRESS } from '@koil-finance/helpers/src/constants';
...
describe('KoilHelpers', function () {
  ...
  sharedBeforeEach('deploy helper', async () => {
    helper = await deploy('KoilHelpers', { args: [pool.vault.address] });
  });
  ...
  describe('queryJoin', () => {
    ...
    it('can query join results', async () => {
      ...
      const data = WeightedPoolEncoder.joinExactTokensInForKPTOut(amountsIn, 0);
  ...
  describe('queryExit', () => {
    ...
    sharedBeforeEach('estimate expected amounts out', async () => {
      ...
      data = WeightedPoolEncoder.exitExactKPTInForTokensOut(bptIn);
    });
    ...
    it('bubbles up revert reasons', async () => {
      ...
      const data = WeightedPoolEncoder.exitExactKPTInForOneTokenOut(bptIn, tooBigIndex);
