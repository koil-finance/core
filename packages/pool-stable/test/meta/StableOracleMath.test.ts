...
import { bn, pct } from '@koil-finance/helpers/src/numbers';
...
import { calculateBptPrice, calculateSpotPrice } from '@koil-finance/helpers/src/models/pools/stable/math';
              ...
              context(`with deltas of magnitude ${delta}`, () => {
                it('computes log spot price with bounded relative error', async () => {
                  for (const balances of balancesOfMagnitude(power, delta)) {
                    ...
                    const logBptPrice = await mock.calcLogBptPrice(amp.mul(AMP_PRECISION), balances, logKPTSupply);
