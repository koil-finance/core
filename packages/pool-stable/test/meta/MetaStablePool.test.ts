...
import { MAX_INT22, MAX_UINT10, MAX_UINT31, MIN_INT22, ZERO_ADDRESS } from '@koil-finance/helpers/src/constants';
import { MINUTE, advanceTime, currentTimestamp, lastBlockNumber } from '@koil-finance/helpers/src/time';
...
import { Sample } from '@koil-finance/helpers/src/models/pools/stable/types';
import Token from '@koil-finance/helpers/src/models/tokens/Token';
import { sharedBeforeEach } from '@koil-finance/common/sharedBeforeEach';
  ...
  describe('oracle', () => {
    ...
    const itUpdatesTheOracleData = (action: PoolHook, lastChangeBlockOffset = 0) => {
      ...
      context('with updated oracle', () => {
        ...
        it('stores the pre-action KPT price', async () => {
          const expectedKPTPrice = await pool.estimateBptPrice(previousBalances, previousTotalSupply);
          const actual = await pool.instance.fromLowResLog(newSample.logBptPrice);

          // The KPT price has twice as much error
          expect(actual).to.equalWithError(expectedKPTPrice, MAX_RELATIVE_ERROR * 2);
        });
    ...
    context('with rate providers', () => {
      const createPoolWithInitialRates = (delta: number) => {
        sharedBeforeEach('mock price rates and deploy pool', async () => {
          rateProviders = await Promise.all(initialBalances.map(() => deploy('pool-utils/MockRateProvider')));
