...
describe('MetaStablePoolFactory', function () {
  ...
  const NAME = 'Koil Meta Stable Pool Token';
  const SYMBOL = 'BMSPT';
  const AMP = 400;
  ...
  sharedBeforeEach('deploy factory & tokens', async () => {
    ...
    rateProviders[0] = (await deploy('pool-utils/MockRateProvider')).address;
  });
  ...
  describe('constructor arguments', () => {
    ...
    it('sets the name', async () => {
      expect(await pool.name()).to.equal('Koil Meta Stable Pool Token');
    });
