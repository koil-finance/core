...
import "@koil-finance/vault/contracts/interfaces/IAsset.sol";
...
contract MultiDistributor is IMultiDistributor, ReentrancyGuard, MultiDistributorAuthorization {
     ...
     */
    function fundDistribution(bytes32 distributionId, uint256 amount) external override nonReentrant(0) {
        Distribution storage distribution = _getDistribution(distributionId);
        // These values being guaranteed to be non-zero for created distributions means we can rely on zero as a
        // sentinel value that marks non-existent distributions.
        require(distribution.duration > 0, "DISTRIBUTION_DOES_NOT_EXIST");
    ...
    function stake(
        ...
        address sender,
        address recipient
    ) external override authenticateFor(sender) nonReentrant(0) {
    ...
    function stakeWithPermit(
        ...
        bytes32 r,
        bytes32 s
    ) external override nonReentrant(0) {
     ...
     */
    function exit(IERC20[] memory stakingTokens, bytes32[] calldata distributionIds) external override nonReentrant(0) {
