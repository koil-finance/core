...
import { PoolSpecialization } from '@koil-finance/helpers/src/utils/types';
import { SwapKind } from '@koil-finance/helpers/src/types';
import { BigNumberish, bn, fp, pct } from '@koil-finance/helpers/src/numbers';
...
import { RawWeightedPoolDeployment, WeightedPoolType } from '@koil-finance/helpers/src/models/pools/weighted/types';
...
export function itBehavesAsWeightedPool(
  ...
  describe('creation', () => {
    context('when the creation succeeds', () => {
      ...
      it('sets the name', async () => {
        expect(await pool.name()).to.equal('Koil Pool Token');
      });

      it('sets the symbol', async () => {
        expect(await pool.symbol()).to.equal('KPT');
      });
  ...
  describe('onJoinPool', () => {
    ...
    context('initialization', () => {
      it('grants the n * invariant amount of KPT', async () => {
    ...
    context('join exact tokens in for KPT out', () => {
      ...
      context('once initialized', () => {
        ...
        it('grants KPT for exact tokens', async () => {
          ...
          // Make sure received KPT is close to what we expect
        ...
        it('can tell how much KPT it will give in return', async () => {
          const minimumBptOut = pct(expectedBptOut, 0.99);
        ...
        it('fails if not enough KPT', async () => {
          // This call should fail because we are requesting minimum 1% more
          const minimumBptOut = pct(expectedBptOut, 1.01);

          await expect(pool.joinGivenIn({ amountsIn, minimumBptOut })).to.be.revertedWith('KPT_OUT_MIN_AMOUNT');
        });
    ...
    context('join token in for exact KPT out', () => {
      const token = 0;
      const bptOut = fp(2);
      ...
      context('once initialized', () => {
        ...
        it('grants exact KPT for token in', async () => {
          const previousBptBalance = await pool.balanceOf(recipient);
        ...
        it('fails if invariant increases more than max allowed', async () => {
          ...
          await expect(pool.joinGivenOut({ bptOut, token })).to.be.revertedWith('MAX_OUT_KPT_FOR_TOKEN_IN');
        });
    ...
    context('join all tokens in for exact KPT out', () => {
      it('fails if not initialized', async () => {
      ...
      context('once initialized', () => {
        ...
        it('grants exact KPT for tokens in', async () => {
          const previousBptBalance = await pool.balanceOf(recipient);
          // We want to join for half the initial KPT supply, which will require half the initial balances
          ...
          // Make sure received KPT equals we expect (since bptOut is given)
          const currentBptBalance = await pool.balanceOf(recipient);
          expect(currentBptBalance.sub(previousBptBalance)).to.be.equal(bptOut);
        });
  ...
  describe('onExitPool', () => {
    ...
    context('exit exact KPT in for one token out', () => {
      const token = 0;
      ...
      it('grants one token for exact bpt', async () => {
        ...
        // Current KPT balance should decrease
      ...
      it('fails if invariant decreases more than max allowed', async () => {
        ...
        await expect(pool.singleExitGivenIn({ bptIn, token })).to.be.revertedWith('MIN_KPT_IN_FOR_TOKEN_OUT');
      });
    ...
    context('exit exact KPT in for all tokens out', () => {
      it('grants all tokens for exact bpt', async () => {
        // Exit with half of the KPT balance
        ...
        // Balances are reduced by half because we are returning half of the KPT supply
        expect(result.amountsOut).to.be.equalWithError(expectedAmountsOut, 0.001);

        // Current KPT balance should have been reduced by half
        expect(await pool.balanceOf(lp)).to.be.equalWithError(bptIn, 0.001);
      });

      it('fully exit', async () => {
        // The LP doesn't own all KPT, since some was locked. They will only be able to extract a (large) percentage
        // of the Pool's balance: the rest remains there forever.
        const totalKPT = await pool.totalSupply();
        const expectedAmountsOut = initialBalances.map((balance) => balance.mul(previousBptBalance).div(totalKPT));
        ...
        // Current KPT balances should be zero due to full exit
    ...
    context('exit KPT in for exact tokens out', () => {
      it('grants exact tokens for bpt', async () => {
        ...
        // KPT balance should have been reduced by half because we are returning half of the tokens
        expect(await pool.balanceOf(lp)).to.be.equalWithError(previousBptBalance.div(2), 0.001);
      });

      it('can tell how much KPT it will have to receive', async () => {
        const amountsOut = initialBalances.map((balance) => balance.div(2));
        const expectedBptIn = previousBptBalance.div(2);
        const maximumBptIn = pct(expectedBptIn, 1.01);
      ...
      it('fails if more BTP needed', async () => {
        ...
        await expect(pool.exitGivenOut({ from: lp, amountsOut, maximumBptIn })).to.be.revertedWith('KPT_IN_MAX_AMOUNT');
      });
  ...
  describe('protocol swap fees', () => {
    ...
    context('with previous swap', () => {
      ...
      it('pays swap protocol fees on join exact tokens in for KPT out', async () => {
      ...
      it('pays swap protocol fees on exit exact KPT in for one token out', async () => {
      ...
      it('pays swap protocol fees on exit exact KPT in for all tokens out', async () => {
      ...
      it('pays swap protocol fees on exit KPT In for exact tokens out', async () => {
