...
import TokensDeployer from '@koil-finance/helpers/src/models/tokens/TokensDeployer';
...
import { bn } from '@koil-finance/helpers/src/numbers';
import { MONTH } from '@koil-finance/helpers/src/time';
...
describe('Internal Balance', () => {
  ...
  let tokens: TokenList, wfuse: Token;
  ...
  sharedBeforeEach('deploy vault & tokens', async () => {
    ...
    wfuse = await TokensDeployer.deployToken({ symbol: 'WFUSE' });

    authorizer = await deploy('Authorizer', { args: [admin.address] });
    vault = await deploy('Vault', { args: [authorizer.address, wfuse.address, MONTH, MONTH] });
  });
  ...
  describe('deposit internal balance', () => {
    ...
    const itHandlesDepositsProperly = (amount: BigNumber, relayed = false) => {
      ...
      it('returns FUSE if any is sent', async () => {
    ...
    context('when the sender is the user', () => {
      ...
      context('when the asset is FUSE', () => {
        ...
        sharedBeforeEach('mint tokens', async () => {
          await wfuse.mint(sender.address, amount);
          await wfuse.approve(vault, amount, { from: sender });
        });

        it('does not take WFUSE from the sender', async () => {
          await expectBalanceChange(
            () =>
              vault.manageUserBalance(
                [{ kind, asset: FUSE_TOKEN_ADDRESS, amount, sender: sender.address, recipient: recipient.address }],
                { value: amount }
              ),
            tokens,
            { account: sender }
        ...
        it('increases the WFUSE internal balance for the recipient', async () => {
          const previousRecipientBalance = await vault.getInternalBalance(recipient.address, [wfuse.address]);
          ...
          const currentRecipientBalance = await vault.getInternalBalance(recipient.address, [wfuse.address]);

          expect(currentRecipientBalance[0].sub(previousRecipientBalance[0])).to.equal(amount);
        });

        it('emits an event with WFUSE as the token address', async () => {
          const receipt: ContractReceipt = await (
            await vault.manageUserBalance(
              [{ kind, asset: FUSE_TOKEN_ADDRESS, amount, sender: sender.address, recipient: recipient.address }],
              { value: amount }
        ...
        it('accepts deposits of both FUSE and WFUSE', async () => {
          const previousRecipientBalance = await vault.getInternalBalance(recipient.address, [wfuse.address]);

          await vault.manageUserBalance(
            [
              { kind, asset: FUSE_TOKEN_ADDRESS, amount, sender: sender.address, recipient: recipient.address },
              { kind, asset: wfuse.address, amount, sender: sender.address, recipient: recipient.address },
            ],
            { value: amount }
          );
        ...
        it('accepts multiple FUSE deposits', async () => {
          const previousRecipientBalance = await vault.getInternalBalance(recipient.address, [wfuse.address]);
        ...
        it('reverts if not enough FUSE was supplied', async () => {
          // Send FUSE to the Vault to make sure that the test fails because of the supplied FUSE, even if the Vault holds
          // enough to mint the WFUSE using its own.
          await forceSendFuse(vault, amount);
    ...
    context('when the sender is a relayer', () => {
      ...
      context('when the relayer is whitelisted by the authorizer', () => {
        ...
        context('when the relayer is allowed to deposit by the user', () => {
          ...
          context('when the asset is FUSE', () => {
            it('returns excess FUSE to the relayer', async () => {
              const amount = bn(100e18);
              ...
              const ethSpent = txFUSE.add(amount).sub(42);
  ...
  describe('withdraw internal balance', () => {
    ...
    context('when the sender is a user', () => {
      ...
      context('when the asset is FUSE', () => {
        ...
        context('when the sender has enough internal balance', () => {
          sharedBeforeEach('deposit internal balance', async () => {
            await wfuse.mint(sender, amount, { from: sender });
            await wfuse.approve(vault, amount, { from: sender });
            await vault.manageUserBalance([
              {
                kind: OP_KIND.DEPOSIT_INTERNAL,
          ...
          it('does not send WFUSE to the recipient', async () => {
            await expectBalanceChange(
              () =>
                vault.manageUserBalance([
                  { kind, asset: FUSE_TOKEN_ADDRESS, amount, sender: sender.address, recipient: recipient.address },
          ...
          it('decreases the WFUSE internal balance for the sender', async () => {
            const previousSenderBalance = await vault.getInternalBalance(sender.address, [wfuse.address]);

            await vault.manageUserBalance([
              { kind, asset: FUSE_TOKEN_ADDRESS, amount, sender: sender.address, recipient: recipient.address },
            ]);

            const currentSenderBalance = await vault.getInternalBalance(sender.address, [wfuse.address]);
          ...
          it('accepts withdrawals of both FUSE and WFUSE', async () => {
            const previousSenderBalance = await vault.getInternalBalance(sender.address, [wfuse.address]);
