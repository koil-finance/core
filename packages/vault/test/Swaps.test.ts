...
import { encodeJoin } from '@koil-finance/helpers/src/models/pools/mockPool';
import * as expectEvent from '@koil-finance/helpers/src/test/expectEvent';
import { Comparison, expectBalanceChange } from '@koil-finance/helpers/src/test/tokenBalance';

import { BatchSwapStep, SwapKind } from '@koil-finance/helpers/src/types';
import { FundManagement, SingleSwap, PoolSpecialization } from '@koil-finance/helpers/src/utils/types';
import { RelayerAuthorization } from '@koil-finance/helpers/src/utils/permit/signatures';
...
import {
  ...
  ZERO_ADDRESS,
  ZERO_BYTES32,
} from '@koil-finance/helpers/src/constants';
...
describe('Swaps', () => {
  ...
  sharedBeforeEach('deploy vault and tokens', async () => {
    tokens = await TokenList.create(['DAI', 'MKR', 'SNX', 'WFUSE']);

    authorizer = await deploy('Authorizer', { args: [admin.address] });
    vault = await deploy('Vault', { args: [authorizer.address, tokens.WFUSE.address, 0, 0] });
  ...
  context('when one of the assets is FUSE', () => {
    ...
    beforeEach(() => {
      tokenAddresses = [FUSE_TOKEN_ADDRESS, tokens.DAI.address];
    });
    ...
    function itSwapsWithFUSECorrectly() {
      ...
      context('when the sender is the trader', () => {
        ...
        it('received FUSE is wrapped into WFUSE', async () => {
          const swaps = [
            {
              poolId: mainPoolId,
              assetInIndex: 0, // FUSE
          ...
          await expectBalanceChange(
            ...
            tokens,
            [
              { account: vault, changes: { WFUSE: 1e18, DAI: -2e18 } },
              { account: trader, changes: { DAI: 2e18 } },
            ]
          );
        });

        it('sent WFUSE is unwrapped into FUSE', async () => {
          const swaps = [
            {
              poolId: mainPoolId,
              assetInIndex: 1,
              assetOutIndex: 0, // FUSE
              amount: bn(1e18),
              userData: '0x',
            },
          ];
          ...
          const receipt: ContractReceipt = await (
            await expectBalanceChange(
              ...
              tokens,
              [
                { account: vault, changes: { WFUSE: -2e18, DAI: 1e18 } },
        ...
        it('emits an event with WFUSE as the token address', async () => {
          ...
          expectEvent.inReceipt(receipt, 'Swap', {
            ...
            tokenOut: tokens.WFUSE.address,
            amountIn: bn(1e18),
            amountOut: bn(2e18),
          });
        });

        it('reverts if less FUSE than required was supplied', async () => {
          const swaps = [
            {
              poolId: mainPoolId,
              assetInIndex: 0, // FUSE
        ...
        it('returns excess FUSE if more FUSE than required was supplied', async () => {
          const swaps = [
            {
              poolId: mainPoolId,
              assetInIndex: 0, // FUSE
      ...
      context('when the sender is an approved relayer', () => {
        ...
        it('returns excess sent FUSE to the relayer', async () => {
          const swaps = [
            {
              poolId: mainPoolId,
              assetInIndex: 0, // FUSE
        ...
        it('returns unreceived FUSE to the relayer', async () => {
          const swaps = [
            {
              poolId: mainPoolId,
              assetInIndex: 1,
