...
import { bn, decimal, fp } from '@koil-finance/helpers/src/numbers';
import { deploy } from '@koil-finance/helpers/src/contract';
import { advanceTime, advanceToTimestamp, currentTimestamp, MINUTE } from '@koil-finance/helpers/src/time';
...
describe('PoolPriceOracle', () => {
  ...
  describe('getPastAccumulator', () => {
    ...
    function itEstimatesPastAccumulatorCorrectly(offset: number) {
      ...
      describe('KPT price', () => itEstimatesPastAccumulatorVariableCorrectly(VARIABLES.KPT_PRICE));
  ...
  describe('queries', () => {
    ...
    const itAnswersQueriesCorrectly = (ascendingAccumulators: boolean) => {
      ...
      describe('getLatest', () => {
        ...
        it('returns the latest KPT price', async () => {
          const actual = await oracle.getLatest(VARIABLES.KPT_PRICE);
      ...
      describe('getPastAccumulators', () => {
        const queries = [
          ...
          { variable: VARIABLES.KPT_PRICE, ago: ago(OLDEST) },
      ...
      describe('getTimeWeightedAverage', () => {
        ...
        const queries = [
          ...
          { variable: VARIABLES.KPT_PRICE, secs, ago: ago(OLDEST + 1) },
