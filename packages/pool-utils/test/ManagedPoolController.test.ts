...
import {
  ...
  ManagedPoolRights,
  BasePoolRights,
} from '@koil-finance/helpers/src/models/pools/weighted/types';
import { deploy } from '@koil-finance/helpers/src/contract';
import { currentTimestamp, MONTH, DAY, HOUR } from '@koil-finance/helpers/src/time';
import Vault from '@koil-finance/helpers/src/models/vault/Vault';
import { encodeInvestmentConfig } from './helpers/rebalance';
import { ZERO_ADDRESS, MAX_UINT256 } from '@koil-finance/helpers/src/constants';
...
sharedBeforeEach('deploy Vault, asset manager, and tokens', async () => {
  ...
  assetManager = await deploy('pool-utils/MockAssetManager', { args: [allTokens.DAI.address] });
});
