...
// File @koil-finance/pool-utils/contracts/interfaces/IRateProvider.sol@v1.0.1
...
// File @koil-finance/vault/contracts/interfaces/IGeneralPool.sol@v1.0.1
...
// File @koil-finance/pool-utils/contracts/BaseGeneralPool.sol@v1.0.1
...
library StableMath {
    ...
    function _calcBptOutGivenExactTokensIn(
        ...
        // If the invariant didn't increase for any reason, we simply don't mint KPT
    ...
    amountOutPercentageExcess -> amountOutBeforeFee -> newInvariant -> amountKPTIn
    */
    ...
    function _getRate(
        ...
        // When calculating the current KPT rate, we may not have paid the protocol fees, therefore
...
// File contracts/StablePoolUserData.sol
...
library StablePoolUserData {
    ...
    enum JoinKind { INIT, EXACT_TOKENS_IN_FOR_KPT_OUT, TOKEN_IN_FOR_EXACT_KPT_OUT }
    enum ExitKind { EXACT_KPT_IN_FOR_ONE_TOKEN_OUT, EXACT_KPT_IN_FOR_TOKENS_OUT, KPT_IN_FOR_EXACT_TOKENS_OUT }
...
contract StablePool is BaseGeneralPool, BaseMinimalSwapInfoPool, IRateProvider {
    ...
    function _onInitializePool(
        ...
        // Set the initial KPT to the value of the invariant.
    ...
    function _doJoin(
        ...
        if (kind == StablePoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_KPT_OUT) {
            return _joinExactTokensInForKPTOut(balances, scalingFactors, userData);
        } else if (kind == StablePoolUserData.JoinKind.TOKEN_IN_FOR_EXACT_KPT_OUT) {
            return _joinTokenInForExactKPTOut(balances, userData);
        } else {
    ...
    function _joinTokenInForExactKPTOut(uint256[] memory balances, bytes memory userData)
        private
        view
    ...
    function _doExit(
        ...
        if (kind == StablePoolUserData.ExitKind.EXACT_KPT_IN_FOR_ONE_TOKEN_OUT) {
            return _exitExactKPTInForTokenOut(balances, userData);
        } else if (kind == StablePoolUserData.ExitKind.EXACT_KPT_IN_FOR_TOKENS_OUT) {
            return _exitExactKPTInForTokensOut(balances, userData);
        } else if (kind == StablePoolUserData.ExitKind.KPT_IN_FOR_EXACT_TOKENS_OUT) {
            return _exitKPTInForExactTokensOut(balances, scalingFactors, userData);
        } else {
            _revert(Errors.UNHANDLED_EXIT_KIND);
        }
    }

    function _exitExactKPTInForTokenOut(uint256[] memory balances, bytes memory userData)
        private
        view
        whenNotPaused
        returns (uint256, uint256[] memory)
 ...
 * particular, it stores reduced-precision versions of the total KPT supply and invariant, which lets us not access
...
contract StableOracleMath {
    ...
    /**
     * @dev Calculates the spot price of token Y and KPT in token X.
     */
    ...
    /**
     * @dev Calculates the price of KPT in token X. `logBptTotalSupply` should be the result of calling
     * `LogCompression.toLowResLog` with the current KPT supply, and `spotPrice` the price of token
    ...
    function _calcLogBptPrice(
        ...
        //                          total supply                                                                     //
        //                                                                                                           //
        // ln(KPT price) = ln(balance X + (spot price Y/X * balance Y)) - ln(totalSupply)                            //
        **************************************************************************************************************/
