...
// File contracts/VaultAuthorization.sol
...
// File contracts/AssetHelpers.sol
...
// File contracts/UserBalance.sol
...
abstract contract UserBalance is ReentrancyGuard, AssetTransfersHandler, VaultAuthorization {
    ...
    function manageUserBalance(UserBalanceOp[] memory ops) external payable override nonReentrant {
        ...
        for (uint256 i = 0; i < ops.length; i++) {
            ...
            if (kind == UserBalanceOpKind.WITHDRAW_INTERNAL) {
                ...
                if (kind == UserBalanceOpKind.DEPOSIT_INTERNAL) {
                    ...
                    // Keep track of all FUSE wrapped into WFUSE as part of a deposit.
                    if (_isFUSE(asset)) {
                        ethWrapped = ethWrapped.add(amount);
                    }
                } else {
                    // Transfers don't support FUSE.
                    _require(!_isFUSE(asset), Errors.CANNOT_USE_FUSE_SENTINEL);
                    IERC20 token = _asIERC20(asset);
        ...
        // Handle any remaining FUSE.
        _handleRemainingFuse(ethWrapped);
    }
...
// File contracts/balances/BalanceAllocation.sol
...
// File contracts/balances/GeneralPoolsBalance.sol
...
// File contracts/AssetManagers.sol
...
// File contracts/PoolBalances.sol
...
abstract contract PoolBalances is Fees, ReentrancyGuard, PoolTokens, UserBalance {
    ...
    function _processJoinPoolTransfers(
        ...
        for (uint256 i = 0; i < change.assets.length; ++i) {
            ...
            if (_isFUSE(asset)) {
                wrappedFuse = wrappedFuse.add(amountIn);
            }
        ...
        _handleRemainingFuse(wrappedFuse);
    }
...
// File contracts/interfaces/IMinimalSwapInfoPool.sol
...
abstract contract Swaps is ReentrancyGuard, PoolBalances {
    ...
    returns (uint256 amountCalculated)
    {
        ...
        // If the asset in is FUSE, then `amountIn` FUSE was wrapped into WFUSE.
        _handleRemainingFuse(_isFUSE(singleSwap.assetIn) ? amountIn : 0);
    }
    ...
    returns (int256[] memory assetDeltas)
    {
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            ...
            if (delta > 0) {
                ...
                if (_isFUSE(asset)) {
                    wrappedFuse = wrappedFuse.add(toReceive);
                }
            } else if (delta < 0) {
...
/**
 * @dev The `Vault` is Koil's core contract. A single instance of it exists for the entire network, and it is the
...
contract Vault is VaultAuthorization, FlashLoans, Swaps {
    ...
    // solhint-disable-next-line func-name-mixedcase
    function WFUSE() external view override returns (IWFUSE) {
        return _WFUSE();
    }
}
