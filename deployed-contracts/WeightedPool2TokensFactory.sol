 ...
 */
interface IVault is IVaultPartial, ISignaturesValidator, ITemporarilyPausable {
    ...
    //    means that Koil governance must approve each individual contract to act as a relayer for the intended
    //    functions.
    ...
    /**
     * @dev Data for `manageUserBalance` operations, which include the possibility for FUSE to be sent and received
     without manual WFUSE wrapping or unwrapping.
     */
    ...
    //
    // Reverts if the FUSE sentinel value is passed.
    //
    // Emits an `InternalBalanceChanged` event.
    //
    //
     ...
     *
     * Because Internal Balance works exclusively with ERC20 tokens, FUSE deposits and withdrawals will use the WFUSE
     * address.
     */
    ...
    // constant product one popularized by Koil V1. Swap costs are smaller compared to general Pools, and are
     ...
     *
     * If exiting a Pool that holds WFUSE, it is possible to receive FUSE directly: the Vault will do the unwrapping. To
     * enable this mechanism, the IAsset sentinel value (the zero address) must be passed in the `assets` array instead
     * of the WFUSE address. Note that it is not possible to combine FUSE and WFUSE in the same exit.
     *
     * `assets` must have the same length and order as the array returned by `getPoolTokens`. This prevents issues when
     * interacting with Pools that register and deregister tokens frequently. If receiving FUSE however, the array must
     * be sorted *before* replacing the WFUSE address with the FUSE sentinel value (the zero address), which means the
     * final `assets` array might not be sorted. Pools with no registered tokens cannot be exited.
     *
     * If `toInternalBalance` is true, the tokens will be deposited to `recipient`'s Internal Balance. Otherwise,
     * an ERC20 transfer will be performed. Note that FUSE cannot be deposited to Internal Balance: attempting to
     ...
     * the `kind` value.
     *
     * `assetIn` and `assetOut` are either token addresses, or the IAsset sentinel value for FUSE (the zero address).
     * Note that Pools never interact with FUSE directly: it will be wrapped to or unwrapped from WFUSE by the Vault.
     *
     ...
     *
     * Returns an array with the net Vault asset balance deltas. Positive amounts represent tokens (or FUSE) sent to the
     * Vault, and negative amounts represent tokens (or FUSE) sent by the Vault. Each delta corresponds to the asset at
     ...
     * or the IAsset sentinel value for FUSE (the zero address). Each entry in the `swaps` array specifies tokens in and
     * out by referencing an index in `assets`. Note that Pools never interact with FUSE directly: it will be wrapped to
     * or unwrapped from WFUSE by the Vault.
     *
     ...
     * `assets` array passed to that function, and FUSE assets are converted to WFUSE.
     *
     ...
     *
     * Note that FUSE cannot be deposited to or withdrawn from Internal Balance: attempting to do so will trigger a
     * revert.
     */
    ...
    /**
     * @dev Returns the Vault's WFUSE instance.
     */
    function WFUSE() external view returns (IWFUSE);
...
// File @koil-finance/solidity-utils/contracts/helpers/LogCompression.sol@v1.0.0
 ...
 * prevent stack-too-deep issues. This is compounded by the fact that Koil contracts typically are memory-intensive,
 ...
 * Adds the {permit} method, which can be used to change an account's ERC20 allowance (see {IERC20-allowance}) by
 * presenting a message signed by the account. By not relying on `{IERC20-approve}`, the token holder account doesn't
 * need to send a transaction, and thus is not required to hold Fuse at all.
 *
 * _Available since v3.4._
 */
 ...
 */
contract KoilPoolToken is ERC20Permit {
    IVault private immutable _vault;
 ...
 */
abstract contract BasePool is IBasePool, BasePoolAuthorization, KoilPoolToken, TemporarilyPausable {
    using WordCodec for bytes32;
    using FixedPoint for uint256;

    uint256 private constant _MIN_TOKENS = 2;

    uint256 private constant _DEFAULT_MINIMUM_KPT = 1e6;
    ...
    /**
     * @dev Returns the minimum KPT supply. This amount is minted to the zero address during initialization, effectively
     * locking it.
     *
    ...
    function onJoinPool(
        ...
        if (totalSupply() == 0) {
            ...
            // On initialization, we lock _getMinimumBpt() by minting it for the zero address. This KPT acts as a
            // minimum as it will never be burned, which reduces potential issues with rounding, and also prevents the
            // Pool from ever being fully drained.
            _require(bptAmountOut >= _getMinimumBpt(), Errors.MINIMUM_KPT);
    ...
    /**
     * @dev Returns the amount of KPT that would be granted to `recipient` if the `onJoinPool` hook were called by the
    ...
    /**
     * @dev Returns the amount of KPT that would be burned from `sender` if the `onExitPool` hook were called by the
    ...
    // Internal hooks to be overridden by derived contracts - all token amounts (except KPT) in these interfaces are
    // upscaled.

    /**
     * @dev Called when the Pool is joined for the first time; that is, when the KPT total supply is zero.
     *
     * Returns the amount of KPT to mint, and the token amounts the Pool will receive in return.
     *
     * Minted KPT will be sent to `recipient`, except for _getMinimumBpt(), which will be deducted from this amount and
     * sent to the zero address instead. This will cause that KPT to remain forever locked there, preventing total BTP
     ...
     *
     * Returns the amount of KPT to mint, the token amounts that the Pool will receive in return, and the number of
     * tokens to pay in protocol swap fees.
     *
     * Implementations of this function might choose to mutate the `balances` array to save gas (e.g. when
     * performing intermediate calculations, such as subtraction of due protocol fees). This can be done safely.
     *
     * Minted KPT will be sent to `recipient`.
     *
     * The tokens granted to the Pool will be transferred from `sender`. These amounts are considered upscaled and will
     * be downscaled (rounding up) before being returned to the Vault.
     *
     ...
     *
     * Returns the amount of KPT to burn, the token amounts for each Pool token that the Pool will grant in return, and
     * the number of tokens to pay in protocol swap fees.
     *
     * Implementations of this function might choose to mutate the `balances` array to save gas (e.g. when
     * performing intermediate calculations, such as subtraction of due protocol fees). This can be done safely.
     *
     * KPT will be burnt from `sender`.
     *
    ...
    function _getAuthorizer() internal view override returns (IAuthorizer) {
        // Access control management is delegated to the Vault's Authorizer. This lets Koil Governance manage which
...
// File @koil-finance/pool-utils/contracts/interfaces/IPriceOracle.sol@v1.0.0
 ...
 * price of the Pool share token (KPT) and invariant. Since the invariant is a sensible measure of Pool liquidity, it
...
interface IPriceOracle {
    ...
    //
    // - KPT_PRICE: the price of the Pool share token (KPT), in units of the first token.
    //   Note that the price is computed *including* the tokens decimals. This means that the KPT price of a Pool with
    //   USDC in which KPT is worth $5 will be 5.0, despite the KPT having 18 decimals and USDC 6.
    //
    // - INVARIANT: the value of the Pool's invariant, which serves as a measure of its liquidity.
    enum Variable { PAIR_PRICE, KPT_PRICE, INVARIANT }
...
// File @koil-finance/pool-utils/contracts/interfaces/IPoolPriceOracle.sol@v1.0.0
...
// File @koil-finance/pool-utils/contracts/oracle/Buffer.sol@v1.0.0
...
// File @koil-finance/pool-utils/contracts/oracle/Samples.sol@v1.0.0
 ...
 * price of KPT (the associated Pool token), and the invariant.
 *
...
library Samples {
    ...
    uint256 internal constant _ACC_LOG_KPT_PRICE_OFFSET = 106;
    uint256 internal constant _INST_LOG_KPT_PRICE_OFFSET = 159;
    ...
    function instant(bytes32 sample, IPriceOracle.Variable variable) internal pure returns (int256) {
        ...
        } else if (variable == IPriceOracle.Variable.KPT_PRICE) {
    ...
    /**
     * @dev Returns `sample`'s instant value for the logarithm of the KPT price.
     */
    function _instLogBptPrice(bytes32 sample) private pure returns (int256) {
        return sample.decodeInt22(_INST_LOG_KPT_PRICE_OFFSET);
    }

    /**
     * @dev Returns `sample`'s accumulator of the logarithm of the KPT price.
     */
    function _accLogBptPrice(bytes32 sample) private pure returns (int256) {
        return sample.decodeInt53(_ACC_LOG_KPT_PRICE_OFFSET);
    }
    ...
    function pack(
        ...
        return
            ...
            instLogBptPrice.encodeInt22(_INST_LOG_KPT_PRICE_OFFSET) |
            accLogBptPrice.encodeInt53(_ACC_LOG_KPT_PRICE_OFFSET) |
...
// File @koil-finance/pool-utils/contracts/oracle/QueryProcessor.sol@v1.0.0
...
// File @koil-finance/pool-utils/contracts/oracle/PoolPriceOracle.sol@v1.0.0
...
library WeightedMath {
    ...
    // So we can round always to the same direction. It is also used to initiate the KPT amount
    // and, because there is a minimum KPT, we round down the invariant.
    ...
    function _calcBptOutGivenExactTokensIn(
        ...
        // KPT out, so we round down overall.
    ...
    function _calcTokenInGivenExactBptOut(
        ...
        /******************************************************************************************
        // tokenInForExactKPTOut                                                                 //
        // a = amountIn                                                                          //
        // b = balance                      /  /    totalKPT + bptOut      \    (1 / w)       \  //
        // bptOut = bptAmountOut   a = b * |  | --------------------------  | ^          - 1  |  //
        // bpt = totalKPT                   \  \       totalKPT            /                  /  //
        // w = weight                                                                            //
        ******************************************************************************************/

        // Token in, so we round up overall.

        // Calculate the factor by which the invariant will increase after minting KPTAmountOut
        uint256 invariantRatio = bptTotalSupply.add(bptAmountOut).divUp(bptTotalSupply);
        _require(invariantRatio <= _MAX_INVARIANT_RATIO, Errors.MAX_OUT_KPT_FOR_TOKEN_IN);
    ...
    function _calcAllTokensInGivenExactBptOut(
        ...
        // aI = amountIn                   /   bptOut   \                                  //
        // b = balance           aI = b * | ------------ |                                 //
        // bptOut = bptAmountOut           \  totalKPT  /                                  //
        // bpt = totalKPT                                                                  //
        ************************************************************************************/

        // Tokens in, so we round up overall.
        uint256 bptRatio = bptAmountOut.divUp(totalKPT);
    ...
    function _calcTokenOutGivenExactBptIn(
        ...
        /*****************************************************************************************
        // exactKPTInForTokenOut                                                                //
        // a = amountOut                                                                        //
        // b = balance                     /      /    totalKPT - bptIn       \    (1 / w)  \   //
        ...
        *****************************************************************************************/
        ...
        // rounds up). Because (totalKPT - bptIn) / totalKPT <= 1, the exponent rounds down.

        // Calculate the factor by which the invariant will decrease after burning KPTAmountIn
        uint256 invariantRatio = bptTotalSupply.sub(bptAmountIn).divUp(bptTotalSupply);
        _require(invariantRatio >= _MIN_INVARIANT_RATIO, Errors.MIN_KPT_IN_FOR_TOKEN_OUT);
    ...
    function _calcTokensOutGivenExactBptIn(
        ...
        /**********************************************************************************************
        // exactKPTInForTokensOut                                                                    //
        // (per token)                                                                               //
        // aO = amountOut                  /        bptIn         \                                  //
        // b = balance           a0 = b * | ---------------------  |                                 //
        // bptIn = bptAmountIn             \       totalKPT       /                                  //
        // bpt = totalKPT                                                                            //
        **********************************************************************************************/
        ...
        uint256 bptRatio = bptAmountIn.divDown(totalKPT);
...
// File contracts/WeightedOracleMath.sol
...
contract WeightedOracleMath {
    ...
    /**
     * @dev Calculates the price of KPT in a token. `logBptTotalSupply` should be the result of calling `toLowResLog`
     * with the current KPT supply.
     *
     * The return value is a 4 decimal fixed-point number: use `LogCompression.fromLowResLog`
     * to recover the original value.
     */
    function _calcLogKPTPrice(
        uint256 normalizedWeight,
        uint256 balance,
        int256 logBptTotalSupply
    ) internal pure returns (int256) {
        // KPT price = (balance / weight) / total supply
        // Since we already have ln(total supply) and want to compute ln(KPT price), we perform the computation in log
        // space directly: ln(KPT price) = ln(balance / weight) - ln(total supply)
...
// File contracts/WeightedPoolUserData.sol
...
// File contracts/WeightedPool2TokensMiscData.sol
 ...
 * reduced-precision versions of the total KPT supply and invariant, which lets us not access nor compute these values
...
// File contracts/WeightedPool2Tokens.sol
    ...
    WeightedOracleMath
{
    ...
    uint256 private constant _MINIMUM_KPT = 1e6;
    ...
    constructor(NewPoolParams memory params)
        ...
        // if the selectors match, preventing accidental errors.
        Authentication(bytes32(uint256(msg.sender)))
        KoilPoolToken(params.name, params.symbol, params.vault)
    ...
    /**
     * @dev Koil Governance can always enable the Oracle, even if it was originally not enabled. This allows for
        ...
        returns (uint256[] memory amountsIn, uint256[] memory dueProtocolFeeAmounts)
    {
        ...
        if (totalSupply() == 0) {
            ...
            // On initialization, we lock _MINIMUM_KPT by minting it for the zero address. This KPT acts as a minimum
            // as it will never be burned, which reduces potential issues with rounding, and also prevents the Pool from
            // ever being fully drained.
            _require(bptAmountOut >= _MINIMUM_KPT, Errors.MINIMUM_KPT);
            _mintPoolTokens(address(0), _MINIMUM_KPT);
            _mintPoolTokens(recipient, bptAmountOut - _MINIMUM_KPT);
     ...
     * Returns the amount of KPT to mint, and the token amounts the Pool will receive in return.
     *
     * Minted KPT will be sent to `recipient`, except for _MINIMUM_KPT, which will be deducted from this amount and sent
     * to the zero address instead. This will cause that KPT to remain forever locked there, preventing total BTP from
    ...
    function _onInitializePool(
        ...
        // Set the initial KPT to the value of the invariant times the number of tokens. This makes KPT supply more
    ...
    function _doJoin(
        ...
        if (kind == WeightedPoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_KPT_OUT) {
            return _joinExactTokensInForKPTOut(balances, normalizedWeights, userData);
        } else if (kind == WeightedPoolUserData.JoinKind.TOKEN_IN_FOR_EXACT_KPT_OUT) {
            return _joinTokenInForExactKPTOut(balances, normalizedWeights, userData);
        } else if (kind == WeightedPoolUserData.JoinKind.ALL_TOKENS_IN_FOR_EXACT_KPT_OUT) {
            return _joinAllTokensInForExactKPTOut(balances, userData);
        } else {
            _revert(Errors.UNHANDLED_JOIN_KIND);
        }
    }

    function _joinExactTokensInForKPTOut(
        uint256[] memory balances,
        uint256[] memory normalizedWeights,
        bytes memory userData
    ) private view returns (uint256, uint256[] memory) {
        (uint256[] memory amountsIn, uint256 minKPTAmountOut) = userData.exactTokensInForBptOut();
        InputHelpers.ensureInputLengthMatch(amountsIn.length, 2);
        ...
        _require(bptAmountOut >= minKPTAmountOut, Errors.KPT_OUT_MIN_AMOUNT);

        return (bptAmountOut, amountsIn);
    }

    function _joinTokenInForExactKPTOut(
        uint256[] memory balances,
        uint256[] memory normalizedWeights,
        bytes memory userData
    ) private view returns (uint256, uint256[] memory) {
    ...
    function _joinAllTokensInForExactKPTOut(uint256[] memory balances, bytes memory userData)
        private
        view
    ...
    {
        // Exits are not completely disabled while the contract is paused: proportional exits (exact KPT in for tokens
    ...
    function _doExit(
        ...
        if (kind == WeightedPoolUserData.ExitKind.EXACT_KPT_IN_FOR_ONE_TOKEN_OUT) {
            return _exitExactKPTInForTokenOut(balances, normalizedWeights, userData);
        } else if (kind == WeightedPoolUserData.ExitKind.EXACT_KPT_IN_FOR_TOKENS_OUT) {
            return _exitExactKPTInForTokensOut(balances, userData);
        } else if (kind == WeightedPoolUserData.ExitKind.KPT_IN_FOR_EXACT_TOKENS_OUT) {
            return _exitKPTInForExactTokensOut(balances, normalizedWeights, userData);
        } else {
            _revert(Errors.UNHANDLED_EXIT_KIND);
        }
    }

    function _exitExactKPTInForTokenOut(
        uint256[] memory balances,
        uint256[] memory normalizedWeights,
        bytes memory userData
    ...
    function _exitExactKPTInForTokensOut(uint256[] memory balances, bytes memory userData)
        private
        view
        returns (uint256, uint256[] memory)
    {
    ...
    function _exitKPTInForExactTokensOut(
        uint256[] memory balances,
        uint256[] memory normalizedWeights,
        bytes memory userData
    ) private view whenNotPaused returns (uint256, uint256[] memory) {
        ...
        (uint256[] memory amountsOut, uint256 maxKPTAmountIn) = userData.bptInForExactTokensOut();
        ...
        _require(bptAmountIn <= maxKPTAmountIn, Errors.KPT_IN_MAX_AMOUNT);
    ...
    /**
     * @dev Updates the Price Oracle based on the Pool's current state (balances, KPT supply and invariant). Must be
    ...
    function _updateOracle(
        ...
        if (miscData.oracleEnabled() && block.number > lastChangeBlock) {
            ...
            int256 logKPTPrice = WeightedOracleMath._calcLogKPTPrice(
                _normalizedWeight0,
                balanceToken0,
    ...
    /**
     * @dev Stores the logarithm of the invariant and KPT total supply, to be later used in each oracle update. Because
     ...
     *
     * This function must be called by all actions that update the invariant and KPT supply (joins and exits). Swaps
    ...
    /**
     * @dev This function returns the appreciation of one KPT relative to the
     * underlying tokens. This starts at 1 when the pool is created and grows over time
     */
    function getRate() public view returns (uint256) {
        // The initial KPT supply is equal to the invariant times the number of tokens.
...
// File contracts/WeightedPool2TokensFactory.sol
