...
/**
 * @dev Pool controller that serves as the owner of a Koil pool, and is in turn owned by
 * an account empowered to make calls on this contract, which are forwarded to the underlyling pool.
 *
 * While the owner of the underlying Koil pool is immutable, ownership of this pool controller
...
abstract contract BaseWeightedPool is BaseMinimalSwapInfoPool {
    ...
    function _doJoin(
        ...
        if (kind == WeightedPoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_KPT_OUT) {
            return _joinExactTokensInForKPTOut(balances, normalizedWeights, scalingFactors, userData);
    ...
    function _doExit(
        ...
        if (kind == WeightedPoolUserData.ExitKind.EXACT_KPT_IN_FOR_ONE_TOKEN_OUT) {
            ...
            return _exitKPTInForExactTokensOut(balances, normalizedWeights, scalingFactors, userData);
        } else {
...
// File contracts/smart/WeightCompression.sol
...
contract ManagedPool is BaseWeightedPool, ReentrancyGuard {
        ...
        )
    {
        ...
        _require(
            getSwapEnabled() || userData.joinKind() == WeightedPoolUserData.JoinKind.ALL_TOKENS_IN_FOR_EXACT_KPT_OUT,
        ...
        )
    {
        ...
        _require(
            getSwapEnabled() ||
                kind == WeightedPoolUserData.ExitKind.EXACT_KPT_IN_FOR_TOKENS_OUT ||
        ...
        returns (uint256 bptAmountIn, uint256[] memory amountsOut)
    {
        ...
        // Since what we're doing is sending out collected management fees, we don't require any KPT in exchange: we
...
// File contracts/smart/BaseManagedPoolFactory.sol
